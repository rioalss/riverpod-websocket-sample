...
class WebsocketService {
  ...
  Future<Either<WebsocketException, Unit>> connect() async {
    ...
    try {
      ...
      final uri = Uri.parse(AppConfig.baseUrlWebsocket);
      ...
      _handleConnectionFailure(WebsocketConnectionException('Network error: ${e.message}'));
      return left(WebsocketConnectionException('Network error: ${e.message}'));
    } on FormatException catch (e) {
      _handleConnectionFailure(WebsocketInvalidUrlException('Invalid URL format: ${e.message}'));
      return left(WebsocketInvalidUrlException('Invalid URL format: ${e.message}'));
    } catch (e) {
  ...
  Either<WebsocketException, Unit> sendPayload(Map<String, dynamic> payload) {
    ...
    try {
      ...
      if (jsonString.length > 16 * 1024) {
        return left(WebsocketPayloadTooLargeException('Payload too large: ${jsonString.length} bytes'));
      }
      ...
      return left(WebsocketSerializationException('JSON serialization failed: ${e.message}'));
    } catch (e) {
  ...
  void _onMessage(dynamic raw) {
    try {
      if (raw is! String) {
        _messageController.addError(WebsocketInvalidMessageException('Expected String, got ${raw.runtimeType}'));
        return;
      }
      ...
      _messageController.addError(WebsocketInvalidMessageException('JSON parse error: $e'));
    } catch (e) {
      _messageController.addError(WebsocketMessageProcessingException('Message processing failed: $e'));
    }
  }

  void _onError(Object error) {
    _updateStatus(WebsocketStatus.error);
    _messageController.addError(WebsocketReceiveException('Receive error: $error'));
    _handleConnectionFailure(WebsocketReceiveException(error.toString()));
  }

  void _onDone() {
    _updateStatus(WebsocketStatus.disconnected);
    _handleConnectionFailure(WebsocketConnectionClosedException('Connection closed by server'));
  }
  ...
  void _scheduleReconnect() {
    ...
    debugPrint('🔄 Reconnecting in ${delay.inSeconds}s (attempt ${_reconnectAttempts + 1}/$_maxReconnectAttempts)');
